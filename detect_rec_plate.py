    ...
    for result in dict_list:
        ...
        if len(result) > 1:
            ...
            orgimg = cv2ImgAddText(orgimg, result, rect_area[0] - height_area, rect_area[1] - height_area - 10,
                                   (0, 255, 0), height_area)
    cars = set(result_str.split())
    for car in cars:
        if car not in resultSet:
            print("车牌号码为：", car)
            resultSet.add(car)
    # car.clear()
    # print("[Draw_result] End")
...
def main_handler(event, context):
    ...
    print(opt)
    model = attempt_load(opt.detect_model, map_location=device)  # 加载检测模型
    # torch.save()
    plate_rec_model = init_model(device, opt.rec_model)  # 初始化模型
    if not os.path.exists(opt.output):  # 判断输出文件夹是否存在，若不存在，创建文件夹
        os.mkdir(opt.output)
    ...
    for pic_ in file_list:
        ...
        file_name, file_type = file  # 获取文件类型
        if file_type in ".jpg .png":  # 对文件夹中的.jpg和.png格式的图片进行检测
            img = cv_imread(pic_)
            if img.shape[-1] == 4:
                img = cv2.cvtColor(img, cv2.COLOR_BGRA2BGR)
            # img = my_letter_box(img)
            dict_list = detect_Recognition_plate(model, img, device, plate_rec_model, opt.img_size)
            ori_img, _ = draw_result(img, dict_list)
        ...
        if file_type in ".mp4 .avi":  # 对文件夹中的.mp4和.avi格式的视频进行检测
            print("Video Number ", videoNumber, " Begin to Print")
            videoNumber += 1
            video = cv2.VideoCapture(pic_)  # 读取视频文件
            ...
            while video.isOpened():
                ...
                if frame is None:
                    break
                if ret == True:
                    dict_list = detect_Recognition_plate(model, frame, device, plate_rec_model, opt.img_size)
                    video_result, _ = draw_result(frame, dict_list)
